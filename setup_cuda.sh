...
then
    echo "No NVIDIA GPU detected. This script is for CUDA-enabled setups only."
    exit 1
fi

echo "Using CUDA-enabled version."
...
sudo docker build -t llm_cleaner-inference:cuda -f ./inference/Dockerfile.cuda ./inference
# Start the containers
sudo -E docker-compose -f docker-compose-cuda.yml up -d --build
